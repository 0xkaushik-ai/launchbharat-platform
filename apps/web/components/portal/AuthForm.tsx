      ...
      <div className="container-lb max-w-md">
        <section className="rounded-2xl border border-line bg-white p-6 shadow-sm sm:p-8">
          ...
          <form className="mt-7 space-y-5" onSubmit={submit}>
            {signup && (
              <label className="block text-sm font-medium text-ink-800">
                Full name
                <input value={name} onChange={(event) => setName(event.target.value)} required autoComplete="name" className="mt-2 w-full rounded-lg border border-line px-3 py-2.5 text-ink-950 outline-none transition focus:border-saffron-500 focus:ring-2 focus:ring-saffron-100" />
              </label>
            )}
            <label className="block text-sm font-medium text-ink-800">
              Email address
              <input value={email} onChange={(event) => setEmail(event.target.value)} required type="email" autoComplete="email" className="mt-2 w-full rounded-lg border border-line px-3 py-2.5 text-ink-950 outline-none transition focus:border-saffron-500 focus:ring-2 focus:ring-saffron-100" />
            </label>
            <label className="block text-sm font-medium text-ink-800">
              Password
              <input value={password} onChange={(event) => setPassword(event.target.value)} required minLength={8} type="password" autoComplete={signup ? "new-password" : "current-password"} className="mt-2 w-full rounded-lg border border-line px-3 py-2.5 text-ink-950 outline-none transition focus:border-saffron-500 focus:ring-2 focus:ring-saffron-100" />
              {signup && <span className="mt-1.5 block text-xs font-normal text-ink-500">Use at least 8 characters.</span>}
            </label>
            {error && <p role="alert" className="rounded-lg bg-red-50 px-3 py-2 text-sm text-red-700">{error}</p>}
            <button disabled={loading} className="w-full rounded-full bg-saffron-600 px-5 py-3 text-sm font-semibold text-white transition hover:bg-saffron-700 disabled:cursor-not-allowed disabled:opacity-60">
          ...
          <p className="mt-6 text-center text-sm text-ink-600">
            ...
            <Link href={`${signup ? "/portal/login" : "/portal/signup"}?next=${encodeURIComponent(next)}`} className="font-semibold text-saffron-600 hover:text-saffron-700">
